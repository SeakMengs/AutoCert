...
func (ac AuthController) Logout(ctx *gin.Context) {
	...
	defer func() {
		if r := recover(); r != nil {
			tx.Rollback()
			util.ResponseFailed(ctx, http.StatusInternalServerError, "Failed to logout", util.GenerateErrorMessages(errors.New("failed to logout")), nil)
			return
		}
	}()

	err = ac.app.Repository.JWT.DeleteToken(ctx, tx, refreshToken)
	if err != nil {
		tx.Rollback()
		util.ResponseFailed(ctx, http.StatusUnauthorized, "", util.GenerateErrorMessages(err), nil)
		return
